...
local function createFinish()
    ...
    addEventHandler('onClientColShapeHit',finishCol,function(e,dim)
        if dim and vehicleIsOurs(e) and stageActive and not hitLocked then
            ...
            triggerServerEvent('waterdiag:freezeVehicle',localPlayer)
            ...
            if currentStage<#stages then
                ...
                setTimer(function() triggerServerEvent('waterdiag:resetStage',localPlayer,currentStage+1) end,3000,1)
            else
                restoreTasWarnings()
...
local function retryStage(reason)
    ...
    executeCommandHandler('record')
    destroyFinish(); triggerServerEvent('waterdiag:freezeVehicle',localPlayer)
    chat(reason..' Retrying this stage; the failed take is NOT saved.',255,120,80)
    setTimer(function()
        fallingRetry=false; hitLocked=false
        triggerServerEvent('waterdiag:resetStage',localPlayer,currentStage)
    end,1800,1)
end
...
local function startTasRecording(index)
    ...
    timer=setTimer(function()
        ...
        triggerServerEvent('waterdiag:goStage',localPlayer,currentStage)
    end,1000,4)
end

addEvent('waterdiag:stageReady',true)
addEventHandler('waterdiag:stageReady',resourceRoot,function(index) startTasRecording(index) end)
addEvent('waterdiag:stageGo',true)
addEventHandler('waterdiag:stageGo',resourceRoot,function(index)
...
addEventHandler('onClientResourceStart',resourceRoot,function()
    chat('Standalone waterjump diagnostic course loaded.',80,220,255)
    chat('Standalone mode: no Race waiting period or Race countdown.',255,220,90)
    ...
    triggerServerEvent('waterdiag:clientReady',localPlayer)
end)
...
addCommandHandler('diagstage',function(_,value)
    ...
    destroyFinish(); triggerServerEvent('waterdiag:freezeVehicle',localPlayer)
    setTimer(function() triggerServerEvent('waterdiag:resetStage',localPlayer,index) end,500,1)
end)
